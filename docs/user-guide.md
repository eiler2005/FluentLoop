...
сообщение короткое и заканчивается за минуту.

```text
🌅 Утро    твои слова с примерами и коротким определением
✍️ День    быстрый дрилл; примерно каждый третий день — «напиши своё предложение»
🌙 Вечер   короткая викторина: выбрать слово по определению
```

Ответы из любого из трёх слотов идут в тот же интервальный повтор, что и
основная сессия. Правильные ответы отодвигают слово дальше; когда слово
освоено, оно «выпускается» (🎓) и уходит из ротации, оставаясь в списке.

### Настройка

- `/setup` — визард: темы, типы лексики (включая fun sets вроде Sci-fi или
  Film noir), размер стартового списка, слов в день. Запускается автоматически
  при первом `/start`.
- `/settings` — поменять время слотов и количество слов в день.
- `/pause` и `/resume` — выключить и включить ежедневные сообщения.

### Свои слова

Отправь боту любое слово или выражение обычным сообщением — оно добавится.
...
Свои слова всегда идут в выдаче первыми.

### Команды словаря

| Команда | Что делает |
|---|---|
| `/today <n>` | показать n карточек прямо сейчас (без аргумента — обычная сессия) |
| `/words` | твой список, счётчик выпущенных, что впереди |
| `/more <word>` | подробная карточка: значение, синонимы, коллокации, примеры |
| `/learned <word>` | пометить как освоенное |
| `/delete <word>` | убрать слово из списка |
