...
FluentLoop работает на двух скоростях, и это главное, что нужно понять:

| | Что это | Сколько времени |
|---|---|---|
| **Слова** | карточки, повторение, квиз — короткие касания несколько раз в день | 0–3 минуты |
| **Уроки** | структурная сессия по теме: грамматика, сценарии, письмо | ~15 минут |

Обе дорожки работают с одной базой и одним интервальным повторением. Слова —
это то, что бот приносит сам три раза в день. Уроки ты запускаешь, когда есть
время.

Что умеет система:

- **Пишет сама три раза в день.** Утром карточки, днём дрилл, вечером квиз —
  в твоей таймзоне, `/pause` выключает.
- **Даёт кнопки вместо команд.** После `/start` под полем ввода появляется
  клавиатура: Cards, Review, Lesson, My words, Add words, Quiz, Stop.
...
- **Тренирует каждый день.** `/today` спрашивает, что запустить: слова или
  урок. Урок — короткая сессия из твоей личной базы: recall, cloze, rewrite,
  translation, free production, cold recall.
...
|---|---|---|
| 🌅 08:00 | Твои слова: фраза, перевод, определение и пример | Просто прочитать |
...
Знание слова раскладывается на три части — форма, значение, употребление — и
карточка несёт все три:
...
Перевод стоит в первой строке: на ней глаз останавливается. Английское
определение под ним, чтобы узнавание не заканчивалось переводом. Пример
...
В ответ придёт готовая карточка — с переводом, определением и примером. Сразу
видно, если перевод неудачный: тогда поправь через `/more` или удали и добавь
заново.
...
`/today` без аргумента спрашивает, что запустить — слова или урок. `/practice`
без аргумента покажет все режимы, сгруппированные по тому, что ты хочешь
потренировать.
...
|---|---|
| `/cards [n]` | Просто посмотреть слова, ничего не отвечая |
| `/review` | Прогнать то, что назрело: 6 шагов, 2–3 минуты |
| `/quiz` | Квиз по требованию, не дожидаясь 19:00 |
| `/practice vocab` | Полный лексический урок на 15 минут |

**Грамматика и ошибки**

| Команда | Когда использовать |
|---|---|
| `/practice grammar` | Разобрать конструкцию |
